...
//    }
    ...
    public String getBookingTest(Model model, @PathVariable String date, @PathVariable int timeIndex){
        ...
        if (this.transactionService.getTransaction(date, timeIndex).isEmpty()){
            model.addAttribute("test", "something wrong");
        }
        model.addAttribute("allTransaction", this.transactionService.getTransaction(date, timeIndex));
        model.addAttribute("allRoomMap", this.roomMapList);
    ...
    @PostMapping("/{date}&{timeIndex}")
    public String editAccount(@ModelAttribute Transaction transaction,
                              Model model) {
        SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");

        transactionService.editTransaction(transaction);

        model.addAttribute("allTransaction", this.transactionService.getTransaction(this.date, this.timeindex));
        return "redirect:/booking/" + this.date + "&" + this.timeindex;
    }
